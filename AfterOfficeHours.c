...
void removeNode(node* node){
  ...
  void insertNode(node *node){
	...
	printf("Inside of insert node\n");
	// don't get pointer of pointers (&head > &node)
	...
	head->prev = node;
	  ...
	  // consult GDB or some other debugger
...
*/
  ...
  node* searchList(size_t size){
	...
	newNode = (node*)(((void*) temp) + size - (size_t)1);
	...
	printf("Temp ->prev = %p\n", temp->prev);
	printf("Node information: beenFreed = %d, size = %x, addr = %p, next = %p\n", newNode->beenFreed, newNode->size, newNode->addr, newNode->next);
	if(head == NULL){
	  head = newNode;
	}
  ...
  void createBlock(size_t size){
    ...
    sizeRequest = size - sizeof(node) - (size_t) 1;
    //Handle overflow
    if(sizeRequest < 0){
	return;
     }
...
void *__realloc_impl(void *ptr, size_t size) {
 ...
 void __free_impl(void *ptr){
   ...
   printf("About to assign freeBlock in free\n");
